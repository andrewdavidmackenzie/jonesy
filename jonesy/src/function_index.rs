...
pub type DwarfFunctionResult = (Vec<FunctionInfo>, Vec<FunctionInfo>, StringTables);
...
pub fn resolve_line_file_path<R: Reader>(
    dwarf: &Dwarf<R>,
    unit: &Unit<R>,
...
/// Find the crate line at a specific address using full DWARF iteration (fallback).
/// This is used when pre-built line tables aren't available.
pub fn get_crate_line_at_address<R: Reader>(
    dwarf: &Dwarf<R>,
    func_start: u64,
    call_site_addr: u64,
    project_context: &ProjectContext,
) -> Result<Option<(u32, Option<u32>)>, gimli::Error> {
    use gimli::ColumnType;
