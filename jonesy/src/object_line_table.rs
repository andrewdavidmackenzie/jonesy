...
pub(crate) struct ObjectLineTable {
    pub entries: Vec<ObjectLineEntry>,
}
...
impl ObjectLineTable {
        ...
        while let Some(header) = units.next()? {
            ...
            if let Some(program) = &unit.line_program {
                ...
                for file_entry in header.file_names() {
                    let full_path = crate::function_index::resolve_line_file_path(
                        dwarf, &unit, file_entry, header,
                    )?;
    ...
    pub fn lookup(&self, address: u64) -> Option<(Option<String>, Option<u32>, Option<u32>)> {
    ...
    pub fn get_crate_line_in_range(
        &self,
        func_start: u64,
        call_site_addr: u64,
