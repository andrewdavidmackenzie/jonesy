...
impl<'a> CallGraph<'a> {
    ...
    pub fn build(
        ...
        let Some((text_addr, text_data)) = crate::sym::get_text_section(macho, buffer) else {
            return Ok(Self {
                edges: HashMap::new(),
            });
        };
    ...
    pub fn build_with_debug_info(
        ...
        let Some((text_addr, text_data)) =
            crate::sym::get_text_section(binary_macho, binary_buffer)
        else {
            return Ok(Self {
                edges: HashMap::new(),
            });
...
pub fn find_callers_with_debug_info(
    ...
    let Some((text_addr, text_data)) = crate::sym::get_text_section(binary_macho, binary_buffer)
    else {
        return Ok(callers);
    };
            ...
            && let Some(operand) = instruction.op_str()
        {
                ...
                && call_target == target_addr
            {
                ...
                if let Some(func) = function_index.find_containing(instruction.address()) {
                    ...
                    if let (Some(f), Some(_crate_path)) = (&file, crate_src_path)
